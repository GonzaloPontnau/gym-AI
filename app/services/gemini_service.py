"""
Service for generating and modifying workout routines using the Gemini API.
Follows dependency injection — receives configuration, not global state.
"""
...
class GeminiRoutineGenerator:
    """Generates and modifies workout routines via Gemini AI."""
    ...
    def __init__(self):
        ...
        if self._configured:
            try:
                ...
                logger.info("Gemini API configured with model %s", settings.GEMINI_MODEL)
            except Exception as e:
                logger.error("Failed to configure Gemini API: %s", e)
                self._configured = False
        else:
            self._model = None
            logger.warning("GEMINI_API_KEY not set — AI service unavailable")
    ...
    def _ensure_configured(self):
        if not self._configured:
            raise ValueError("Gemini API is not configured. Cannot process AI requests.")
    ...
    async def create_initial_routine(self, request: RoutineRequest) -> Routine:
        ...
        try:
            logger.info("Sending initial routine request to Gemini...")
            ...
            raise ValueError(f"Error generating routine with Gemini: {e}")
    ...
    async def modify_routine(self, current_routine: Routine, user_request: str) -> Routine:
        ...
        try:
            ...
            raise ValueError(f"Error modifying routine with Gemini: {e}")
    ...
    async def explain_routine_changes(
        ...
        try:
            ...
            raise ValueError(f"Error generating explanation with Gemini: {e}")